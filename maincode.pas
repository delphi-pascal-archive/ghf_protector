/////////////////////////////////////////
// GHF - GPcH, Holy_Father, FEUERRADER //
//              Protector              //
/////////////////////////////////////////
//    Based on Morphine and AHPack     //
/////////////////////////////////////////
//  Packing only first section in EXE  //
/////////////////////////////////////////
// Protected programs working only on  //
// Windows based on NT (NT,2k,XP,2003) //
/////////////////////////////////////////
...
uses
  ...
  Dialogs, aPLib, StdCtrls, ComCtrls, PE_Files, Menus,ExtCtrls, shellapi, morphine,
...
begin
 ...
 frmMain.aPLib.Pack;
  ...
  if frmMain.aPLib.Length = 0 then Exit;
    ...
    frmMain.AddLog('Size of section: '+inttostr(frmMain.CurFileSz)+' byte(s)',0);
    frmMain.AddLog('Packed of section: '+inttostr(PACKEDSECTION)+' byte(s)',0);
    frmMain.AddLog('Size of depacker: '+inttostr(GetLoaderSize(dword(@PE_Loader)))+' byte(s)',0);
    frmMain.AddLog(FormatFloat('Ratio: ##%', (packedsection*100)/frmMain.CurFileSz),0);
...
if (InputFileName='') or (fileexists(InputFileName)=false) then
   begin
   ClearLog;
   AddLog('Cannot open file',2);
     Exit;
   end;
 hFile:=CreateFileA(pchar(InputFileName), GENERIC_READ + GENERIC_WRITE, FILE_SHARE_READ + FILE_SHARE_WRITE, NIL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
  ...
  ReadFile(hFile,EXE,2,bread,NIL);
   ...
   if PE_HEADER.IMAGE_NT_SIGNATURE<>$00004550 then
    begin
   ClearLog;
   AddLog('Invalid executable file',2);
      CloseHandle(hFile);
      Exit;
    end;
   for i:=1 to PE_HEADER.FILE_HEADER.NumberOfSections Do
   ...
   SECTION_HEADER[num].SizeOfRawData:=GetFileSize(hFile,nil)-SECTION_HEADER[num].PointerToRawData;
...
PE_HEADER.OPTIONAL_HEADER.IMAGE_DIRECTORY_ENTRIES.IMPORT.Size:=$b1;
   ...
   CloseHandle(hFile);
   frmMain.StatusBar1.Panels.Items[0].Text:='Optimizing...';
   AddLog('Optimizing...',0);
   ...
   pe.Free;
   AddLog('File successfully packed',0);

   frmMain.StatusBar1.Panels.Items[0].Text:='File successfully packed';
end;
...
begin
  ...
  frmMain.StatusBar1.Panels.Items[0].Text:='Working';
  ...
  if (rbProtect.Checked or rbFullProtect.Checked) then begin
    frmMain.StatusBar1.Panels.Items[0].Text:='Protect....';
    Protect(txtFileName.Text);
  end;
  frmMain.StatusBar1.Panels.Items[0].Text:='Ready';
...
begin
  frmMain.dlgOpen.Title:='Open EXE File';
  frmMain.dlgOpen.Filter:='EXE Files (*.exe)|*.exe';
...
begin
  if (MessageDlg('Are you sure, do you want to set all settings to default?',mtConfirmation,[mbYes, mbNo],0)=mrYes) then begin
    txtFileName.Text:='';
...
begin
  frmMain.dlgOpen.Title:='Open GHF Project';
  frmMain.dlgOpen.Filter:='GHF Projects (*.ghf)|*.ghf';
  if frmMain.dlgOpen.Execute then begin
    ...
    if (strProject.sPacking and strProject.sProtection) then rbFullProtect.Checked:=True;
...
var
  ...
  f: file of ProtectorProject;
begin
  frmMain.dlgSave.Title:='Save GHF Project';
  frmMain.dlgSave.Filter:='GHF Projects (*.ghf)|*.ghf';
  if frmMain.dlgSave.Execute then begin
    if (ExtractFileExt(frmMain.dlgSave.FileName)='') then frmMain.dlgSave.FileName:=frmMain.dlgSave.FileName+'.ghf';
...
begin
  if not (ShellExecute(frmMain.Handle,'open',pchar(Application.HelpFile),
  pchar(''),pchar(ExtractFilePath(ParamStr(0))),1)=42) then
  ShowMessage('File "'+Application.HelpFile+'" not found in program directory');
end;
