...
asm
  ...
  //but we modify esi a lot and we shouldn't do this
  //but as bad boys we can fix this up, cuz we know we left esp and it has right value
  //so add sum 010h for DllMain params + ret addr and here we go
  mov esi,esp
  mov [esi+004h],ecx            //change DllMain.hinstDLL
  add esi,010h
    ...
    begin
     ...
     begin
      ...
      OverlayPresent:=(AfterImageOverlays<>nil) and (AfterImageOverlaysSize>0);
       ...
       begin
        frmMain.AddLog('.tls section is present',0);
        frmMain.AddLog('Original .tls section size: '+IntToStr(TlsSectionSize),0);
       end else frmMain.AddLog('.tls section not present',1);

       if ExportSectionPresent then
       begin
        if not DynamicDLL then
        begin
         frmMain.AddLog('Export section is present',0);
         frmMain.AddLog('Original export section size: '+IntToStr(ExportSectionSize),0);
        end else frmMain.AddLog('Dynamic DLL - export section not used',1);
       end else frmMain.AddLog('Export section not present',1);

       if ResourceSectionPresent then
       begin
        if SaveIcon then frmMain.AddLog('Resource section present',0)
        else frmMain.AddLog('Resource section present but not used',1);
       end else frmMain.AddLog('Resource section not present',1);

       if OverlayPresent then
       begin
        if SaveOverlay then frmMain.AddLog('Overlay data present',0)
        else frmMain.AddLog('Overlay data present but not used',1);
       end else frmMain.AddLog('Overlay data not present',1);
      ...
      begin
       ...
       if not ResourceSectionPresent then
         frmMain.AddLog('Resource section has no icon or XP manifest',1);
      end;
      ...
      begin
       ...
       frmMain.AddLog('Loader size: '+IntToStr(LoaderSize),0);
       ...
       begin
        ...
        frmMain.AddLog('Encryption key size: '+IntToStr(KeySize),0);
        Key:=MyAlloc(KeySize);
        if Key<>nil then
        begin
         ...
         frmMain.AddLog('Building .text section',0);
         ...
         frmMain.AddLog('.text section virtual address: '+IntToHex(CodeSection.VirtualAddress,8),0);
         frmMain.AddLog('.text section virtual size: '+IntToHex(CodeSection.Misc.VirtualSize,8),0);

         frmMain.AddLog('Building .idata section',0);
         ...
         frmMain.AddLog('.idata section virtual address: '+IntToHex(ImportSection.VirtualAddress,8),0);
         frmMain.AddLog('.idata section virtual size: '+IntToHex(ImportSection.Misc.VirtualSize,8),0);

         // .tls Section
         if TlsSectionPresent then
         begin
          frmMain.AddLog('Building .tls section',0);
          ...
          frmMain.AddLog('.tls section virtual address: '+IntToHex(TlsSection.VirtualAddress,8),0);
          frmMain.AddLog('.tls section virtual size: '+IntToHex(TlsSection.Misc.VirtualSize,8),0);
         end;

         if ExportSectionPresent then
         begin
          frmMain.AddLog('Building .edata section',0);
          ...
          frmMain.AddLog('export section virtual address: '+IntToHex(ExportSection.VirtualAddress,8),0);
          frmMain.AddLog('export section virtual size: '+IntToHex(ExportSection.Misc.VirtualSize,8),0);
         end;

         if ResourceSectionPresent then
         begin
          frmMain.AddLog('Building .rsrc section',0);
          ...
          frmMain.AddLog('resource section virtual address: '+IntToHex(ResourceSection.VirtualAddress,8),0);
          frmMain.AddLog('resource section virtual size: '+IntToHex(ResourceSection.Misc.VirtualSize,8),0);
         end;
 
         NtHeaders.OptionalHeader.SizeOfImage:=CurVirtAddr;

         frmMain.AddLog('Building import descriptor ...',0);
         ...
         frmMain.AddLog('Building polymorphic part ...',0);
         TrashSize:=KeyPtr;

         frmMain.AddLog('Key address: '+IntToHex(KeyPtr,8),0);
         frmMain.AddLog('Loader address: '+IntToHex(LoaderPtr,8),0);
         frmMain.AddLog('Trash bytes size: '+IntToStr(TrashSize),0);
         frmMain.AddLog('Trash2 bytes size: '+IntToStr(Trash2Size),0);
         ...
         begin
          ...
          frmMain.AddLog('Executable entry point: '+IntToHex(NtHeaders.OptionalHeader.AddressOfEntryPoint,8),0);
          ...
          begin
           ...
           frmMain.AddLog('Generating init code ...',0);
           ...
           begin
            ...
            if not Quiet then
            begin
             frmMain.AddLog('Size of new stuff: '+IntToStr(TotalFileSize-MainRealSize)+' bytes',0);
             LogCnt:=0;
            ...
            frmMain.AddLog('File successfully protected',0);
            frmMain.StatusBar1.Panels.Items[0].Text:='File successfully protected';
